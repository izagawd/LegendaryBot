...
public class Dialogue
{
    ...
    public TimeSpan TimeoutTimespan => new(0, 1, 0);
        ...
        params DiscordActionRowComponent[] discordActionRows)
    {
        ...
        if (isLast && RemoveButtonsAtEnd && DecisionArgument is null) messageBuilder.ClearComponents();
        ...
        if (_interaction is not null && _message is null)
        {
            ...
            if (isLast && RemoveButtonsAtEnd && DecisionArgument is null) responseBuilder.ClearComponents();
    ...
    private void HandleInteractionResult(InteractivityResult<ComponentInteractionCreatedEventArgs> args)
    {
        ...
        if (answer == "skip") _skipped = true;

        if (_skipped) _finished = true;
    }
    ...
    public Task<DialogueResult> LoadAsync(DiscordUser user, DiscordInteraction interaction)
    {
        ...
        return LoadAsync(user, interaction, null);
    }
    ...
    /// <returns></returns>
    private async Task<DialogueResult> LoadAsync(DiscordUser user, DiscordInteraction? interaction,
