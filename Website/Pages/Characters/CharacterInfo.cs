...
public partial class CharacterInfo
{
    ...
    {
        [JsonIgnore] public string StatName { get; internal set; }

        [JsonPropertyName("1")] public int TypeId { get; set; }

        [JsonPropertyName("2")] public bool IsPercentage { get; set; }

        [JsonPropertyName("3")] public float Value { get; set; }

        [JsonPropertyName("4")] public bool IsMainStat { get; set; }
    }

    public class GearDto
    {
        [JsonIgnore] public string GearName { get; internal set; }

        [JsonPropertyName("2")] public long Id { get; set; }

        [JsonPropertyName("3")] public int RarityNum { get; set; }

        [JsonPropertyName("4")] public int Number { get; set; }
        ...
        public string? EquippedCharacterImageUrl =>
            EquippedCharacterTypeId is not null
                ? $"{Information.CharactersImagesDirectory}/{EquippedCharacterTypeId}.png"
                : null;

        [JsonPropertyName("6")] public int? EquippedCharacterTypeId { get; set; }

        [JsonIgnore] public string ImageUrl => $"{Information.GearsImagesDirectory}/{TypeId}.png";

        [JsonPropertyName("8")] public string? OriginalOwnerImageUrl { get; set; }
    ...
    public class BlessingDto
    {
        ...
        [JsonPropertyName("2")] public string Description { get; set; }

        [JsonPropertyName("3")] public int RarityNum { get; set; }

        [JsonPropertyName("4")] public string Name { get; set; }

        [JsonIgnore] public string ImageUrl => $"{Information.BlessingImagesDirectory}/{TypeId}.png";

        [JsonPropertyName("6")] public int Stacks { get; set; }

        [JsonPropertyName("7")] public int RemainingStacks { get; set; }
    }
    ...
    public class CharacterInfoDto
    {
        ...
        public void Setup()
        {
            foreach (var i in AllGears) i.GearName = GearNameByTypeId[i.TypeId];

            foreach (var i in AllGears.SelectMany(j => j.GearStats)) i.StatName = GearStatNameByTypeId[i.TypeId];
        }
    }
}