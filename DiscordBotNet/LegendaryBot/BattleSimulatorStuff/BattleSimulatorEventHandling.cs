...
public partial class BattleSimulator
{
    ...
    private static void SetupBattleEventDelegatorStuff()
    {
        ...
        foreach (var i in Assembly.GetExecutingAssembly().GetTypes().Where(j => !j.IsAbstract && j.GetInterfaces().Contains(typeof(IBattleEventListener))))
        {
                         ...
                         .Where(j => j.attribute is not null))
            {
                ...
                var cache = new EventMethodDetails(CreateBattleEventMethodFrom(j.method), j.attribute!, parameter.ParameterType);
    ...
    public static BattleEventMethod CreateBattleEventMethodFrom(MethodInfo methodInfo)
    {
        ...
        {
            throw new Exception("Method must return nothing");
        }
