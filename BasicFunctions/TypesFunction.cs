...
namespace BattleManagemen.LegendaryBot;
...
{
    private static readonly ConcurrentDictionary<Type, object> _defaultObjectsContainer = new();


    public static ConcurrentDictionary<Assembly, Type[]> _allAssemblyTypes = new();

    private static readonly ConcurrentDictionary<Type, List<Type>> _subClassesCache = [];
    ...
    private static void RegisterDefaultObjects(Assembly assembly)
    {
                     ...
                     .Where(j => j.GetCustomAttribute<DefaultObjectRegistererAttribute>() is not null))
        {
            ...
            var theObjects = (IEnumerable<object>)j.Invoke(null, null)!;
            foreach (var k in theObjects) _defaultObjectsContainer[k.GetType()] = k;
        }
    ...
    private static void RegisterAssemblies(IEnumerable<Assembly> assemblies)
    {
        ...
        foreach (var i in arrayed) AddAllAssemblyTypes(i);
        foreach (var i in arrayed) RegisterDefaultObjects(i);
    }
    ...
    {
        foreach (var i in GetTypesThatSubclassesList(type)) yield return i;
    }
    ...
    public static IEnumerable<Type> GetCachedAssemblyTypes(Assembly assembly)
    {
        ...
        foreach (var i in types) yield return i;
    }
    ...
    public static object GetDefaultObject(Type type)
    {
        if (!_defaultObjectsContainer.TryGetValue(type, out var o))
        {
            ...
            var createdObject = Activator.CreateInstance(type,
                ...
                null, [], null);
            if (createdObject is null) throw new Exception("Something went wrong");
