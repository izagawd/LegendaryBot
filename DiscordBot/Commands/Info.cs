...
{
    public static async Task<Image<Rgba32>> GetInfoAsync(UserData userData, DiscordUser? user = null)
    {
        var Experience = userData.Experience;
        if (user is null)
            user = await DiscordBot.Client.GetUserAsync(userData.DiscordId);
        else if (user.Id != userData.DiscordId) throw new Exception("discord user's ID does not match user data's id");
        ...
        image.Mutate(ctx =>
        {
            ...
            var gottenExp = levelBarMaxLevelWidth * (Experience / (userData.GetRequiredExperienceToNextLevel() * 1.0f));
            ...
            ctx.DrawText($"{Experience}/{userData.GetRequiredExperienceToNextLevel()}", font,
                SixLabors.ImageSharp.Color.Black,
                new PointF(140, levelBarY + 2));
            ctx.DrawText($"Adventurer Level {userData.AdventurerLevel}", font, SixLabors.ImageSharp.Color.Black,
